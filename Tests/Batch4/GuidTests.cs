...
{
    [TestFixture(TestNameFormat = "GuidTests - {0}")]
    public class GuidTests
    {
        [Test]
        public void TypePropertiesAreCorrect()
        {
            Assert.AreEqual("System.Guid", typeof(Guid).FullName);
            Assert.False(typeof(Guid).IsClass);
            Assert.True(typeof(IComparable<Guid>).IsAssignableFrom(typeof(Guid)));
            Assert.True(typeof(IEquatable<Guid>).IsAssignableFrom(typeof(Guid)));
            object o = new Guid();
            ...
            Assert.True(interfaces.Contains(typeof(IComparable<Guid>)));
            Assert.True(interfaces.Contains(typeof(IEquatable<Guid>)));
        ...
        public void ToByteArrayWorks()
        {
            ...
            Assert.AreEqual(new byte[] { 0x84, 0x40, 0xf8, 0x54, 0x0c, 0x0b, 0x43, 0x55, 0x97, 0x22, 0x16, 0x08, 0xd6, 0x2e, 0x8f, 0x87 }, g.ToByteArray());
        }
    }
}
