...
return result;}},constructor:function(name){this.name=name;if(Bridge.CultureInfo.cultures[name]){Bridge.copy(this,Bridge.CultureInfo.cultures[name],["englishName","nativeName","numberFormat","dateTimeFormat"]);}else{Bridge.CultureInfo.cultures[name]=this;}},getFormat:function(type){switch(type){case Bridge.NumberFormatInfo:return this.numberFormat;case Bridge.DateTimeFormatInfo:return this.dateTimeFormat;default:return null;}},clone:function(){return Bridge.copy(new Bridge.CultureInfo(this.name),this,["englishName","nativeName","numberFormat","dateTimeFormat"]);}});Bridge.CultureInfo.cultures={};Bridge.CultureInfo.invariantCulture=Bridge.merge(new Bridge.CultureInfo("en-US"),{englishName:"English (United States)",nativeName:"English (United States)",numberFormat:Bridge.NumberFormatInfo.invariantInfo,dateTimeFormat:Bridge.DateTimeFormatInfo.invariantInfo});Bridge.CultureInfo.setCurrentCulture(Bridge.CultureInfo.invariantCulture);
...
return part;});},parse:function(value,provider,silent){var dt=Date.parse(value);if(!isNaN(dt)){return new Date(dt);}
return this.parseExact(value,null,provider,silent);},parseExact:function(str,format,provider,silent){if(!format){format=["G","g","F","f","D","d","R","r","s","U","u","O","o","Y","y","M","m","T","t"];}
...
return null;},tryParse:function(value,provider,result){result.v=this.parse(value,provider,true);if(result.v==null){result.v=new Date(-864e13);return false;}
...
else if(Bridge.isFunction(obj.clear)){obj.clear();}},indexOf:function(arr,item){var i,ln;for(i=0,ln=arr.length;i<ln;i++){if(arr[i]===item){return i;}}
...
else if(Bridge.isFunction(obj.removeAt)){obj.removeAt(index);}}};Bridge.Array=array;})();
