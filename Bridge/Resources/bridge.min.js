...
v_str=tmp+v_str;}
this.exp=exp;if(exp<-1){throw new Bridge.OverflowException();}
...
return ret;};Decimal.MidpointRounding={AwayFromZero:0,ToEven:1};Decimal.word_base=16;Decimal.decimal_digits_per_word=7;Decimal.n=4;Decimal.decimal_digits=Decimal.decimal_digits_per_word*Decimal.n;Decimal.one_word=Decimal.pow(10.0,Decimal.decimal_digits_per_word);Decimal.two_words=Decimal.pow(10.0,(Decimal.decimal_digits_per_word*2));Decimal.initConstants=function(){var tmp="\\d{";tmp+=Decimal.decimal_digits_per_word;tmp+="}$";Decimal.regexp_digits=new RegExp(tmp);Decimal.one_word_zeros="";for(var i=0;i<Decimal.decimal_digits_per_word-1;++i){Decimal.one_word_zeros+="0";}
...
return(this.is_minus?"-":"")+str_sig;};Decimal.prototype.toFloat=function(){return parseFloat(this.toString());};Decimal.fromData=function(sig,exp,is_minus){var obj=new Decimal();var orig_len=sig.length;var valid_num=Decimal.countValidNum(sig);obj.exp=(exp<valid_num?Decimal.decimal_digits+exp-valid_num:Decimal.decimal_digits);obj.is_minus=is_minus;if(obj.exp<-1){throw new Bridge.OverflowException();}
...
var is_minus=(this.is_minus!=another_decimal.is_minus);return Decimal.fromData(sig,this.exp+another_decimal.exp,is_minus);};Decimal.prototype.div=function(another){var another_decimal=(another instanceof Decimal?another:new Decimal(another));if(another_decimal.isZero()){throw new Bridge.DivideByZeroException();}
