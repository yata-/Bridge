...
(function(){var ROOT=this;var Decimal=function(v,provider){if(this.constructor!=Decimal){return new Decimal(v);}
...
provider=provider||Bridge.CultureInfo.getCurrentCulture();var nfInfo=provider&&provider.getFormat(Bridge.NumberFormatInfo);var v_str="";if(typeof v==='string'){if(nfInfo&&nfInfo.numberDecimalSeparator!=="."){v=v.replace(nfInfo.numberDecimalSeparator,".");}
if(!/^\s*[+-]?(\d+|\d*\.\d+)(e[+-]?\d+)?\s*$/.test(v)){throw new Bridge.FormatException();}
v_str=v.replace(/\s/g,'');}else{v_str=v.toString();}
this.is_minus=false;var ch=v_str.charAt(0);if(ch=="+"||ch=="-"){this.is_minus=(ch=="-");v_str=v_str.substring(1);}
if(v_str.length>29&&v_str.indexOf(".")>-1){v_str=v_str.substring(0,29);var lastChar=v_str.charAt(v_str-1);if(lastChar=="."){v_str=v_str.substring(0,28);}}
...
exp+=back_zero_count;}
if(v_str.length-exp>Decimal.decimal_digits){throw new Bridge.OverflowException();}
...
Decimal.$$name="Bridge.Decimal";Decimal.prototype.$$name="Bridge.Decimal";Decimal.$$inherits=[];Bridge.Class.addExtend(Decimal,[Bridge.IComparable,Bridge.IFormattable,Bridge.IComparable$1(Decimal),Bridge.IEquatable$1(Decimal)]);Decimal.getDefaultValue=function(){return Decimal(0);};Decimal.lift=function(d){return d==null?null:Decimal(d);};Decimal.pow=function(n,m){var ret=1;for(var i=0;i<m;++i){ret*=n;}
return ret;};Decimal.MidpointRounding={AwayFromZero:0,ToEven:1,Floor:2,Ceil:3};Decimal.decimal_digits_per_word=7;Decimal.n=4;Decimal.decimal_digits=Decimal.decimal_digits_per_word*Decimal.n;Decimal.one_word=Decimal.pow(10.0,Decimal.decimal_digits_per_word);Decimal.two_words=Decimal.pow(10.0,(Decimal.decimal_digits_per_word*2));Decimal.initConstants=function(){var tmp="\\d{";tmp+=Decimal.decimal_digits_per_word;tmp+="}$";Decimal.regexp_digits=new RegExp(tmp);Decimal.one_word_zeros="";for(var i=0;i<Decimal.decimal_digits_per_word-1;++i){Decimal.one_word_zeros+="0";}
Decimal.zeros="";for(var i=0;i<Decimal.decimal_digits-1;++i){Decimal.zeros+="0";}
Decimal.constants_initialized=true;}
Decimal.prototype.toString=function(format,provider){if(!format&&!provider){if(this.isZero()){return"0";}
...
return Bridge.Int.format(this.toFloat(),format,provider);};Decimal.prototype.toFloat=function(){return parseFloat(this.toString());};Decimal.prototype.format=function(fmt,provider){return this.toString(fmt,provider);};Decimal.fromData=function(sig,exp,is_minus){var obj=new Decimal();var orig_len=sig.length;var valid_num=Decimal.countValidNum(sig);obj.exp=(exp<valid_num?Decimal.decimal_digits+exp-valid_num:Decimal.decimal_digits);obj.is_minus=is_minus;if(obj.exp<-1){throw new Bridge.OverflowException();}
...
return obj;};Decimal.prototype.abs=function(){var r=new Decimal(this);r.is_minus=false;return r;};Decimal.prototype.floor=function(){return Decimal.roundInternal(this,0,this.is_minus?Decimal.MidpointRounding.Ceil:Decimal.MidpointRounding.Floor);};Decimal.prototype.ceil=function(){return Decimal.roundInternal(this,0,this.is_minus?Decimal.MidpointRounding.Floor:Decimal.MidpointRounding.Ceil);};Decimal.prototype.trunc=function(){return Decimal.roundInternal(this,0,Decimal.MidpointRounding.Floor);};Decimal.round=function(obj,digits,mode){if(mode!=Decimal.MidpointRounding.AwayFromZero&&mode!=Decimal.MidpointRounding.ToEven){throw new Bridge.ArgumentException("mode");}
return Decimal.roundInternal(obj,digits,mode);}
Decimal.roundInternal=function(obj,digits,mode){var ret=new Decimal(obj);var pos=obj.exp-digits;if(pos<=0||pos>Decimal.decimal_digits||obj.isZero()){return ret;}
var last_word_pos=Math.floor((pos-1)/Decimal.decimal_digits_per_word);var last_digit_pos=(pos-1)%Decimal.decimal_digits_per_word;var round_word_pos=Math.floor(pos/Decimal.decimal_digits_per_word);var round_digit_pos=pos%Decimal.decimal_digits_per_word;var tmp=Decimal.pow(10,round_digit_pos);var round_up=false;var tmp2=Decimal.pow(10,last_digit_pos+1);var zero_flag=true;for(var i=0;i<last_word_pos;++i){if(obj.sig[i]){zero_flag=false;ret.sig[i]=0;}}
var last_num=obj.sig[last_word_pos]%tmp2;if(mode==Decimal.MidpointRounding.Floor){;}else if(mode==Decimal.MidpointRounding.Ceil&&!(last_num==0&&zero_flag)){round_up=true;}else{var last_digit_is_even=(Math.floor(obj.sig[round_word_pos]/tmp)%2==0);if(last_num>tmp2/2){round_up=true;}else if(last_num==tmp2/2){if(!zero_flag||!(last_digit_is_even&&mode==Decimal.MidpointRounding.ToEven)){round_up=true;}}}
...
return ret;};Decimal.prototype.compareTo=function(another){var another_decimal=(another instanceof Decimal?another:new Decimal(another));var this_sign=this.is_minus?-1:1;if(this.is_minus!=another_decimal.is_minus){return this_sign;}
...
var ret=abs_larger.absAddSub(abs_smaller,is_abs_sub);ret.is_minus=ret_is_minus;return ret;};Decimal.prototype.add=function(another){return this.addSub(another,false);};Decimal.prototype.sub=function(another){return this.addSub(another,true);};Decimal.prototype.isZero=function(){for(var i=0;i<Decimal.n;++i){if(this.sig[i]){return false;}}
return true;};Decimal.prototype.mul=function(another){var another_decimal=(another instanceof Decimal?another:new Decimal(another));if(this.isZero()||another_decimal.isZero()){return Decimal(0);}
...
(another_v-(this_v-added)+Decimal.n+1)*Decimal.decimal_digits_per_word);return Decimal.fromData(result_sig,result_exp,this.is_minus!=another_decimal.is_minus);};Decimal.prototype.mod=function(other){var other_decimal=(other instanceof Decimal?other:new Decimal(other));var a=this.abs();var b=other_decimal.abs();var r=a.sub(a.div(b).trunc().mul(b));r.is_minus=this.is_minus;return r;};Decimal.prototype.neg=function(){var r=new Decimal(this);if(!r.isZero()){r.is_minus=!r.is_minus;}
return r;};Decimal.prototype.inc=function(){return this.addSub(1,false);};Decimal.prototype.dec=function(){return this.addSub(1,true);};Decimal.prototype.sign=function(){return this.isZero()?0:(this.is_minus?-1:1);};Decimal.prototype.clone=function(){return new Bridge.Decimal(this);};Decimal.prototype.ne=function(v){return!!this.compareTo(v);};Decimal.prototype.lt=function(v){return this.compareTo(v)<0;};Decimal.prototype.lte=function(v){return this.compareTo(v)<=0;};Decimal.prototype.gt=function(v){return this.compareTo(v)>0;};Decimal.prototype.gte=function(v){return this.compareTo(v)>=0;};Decimal.prototype.equals=function(v){return!this.compareTo(v);};Decimal.prototype.equalsT=function(v){return!this.compareTo(v);};Decimal.prototype.getHashCode=function(){var n=(this.is_minus*397+this.exp)|0;for(var i=0;i<this.sig.length;i++){n=(n*397+this.sig[i])|0;}
return n;};Decimal.toInt=function(v){if(!v){return null;}
var i=Bridge.Int.trunc(v.toFloat());if(!Bridge.Int.instanceOf(i)){throw new Bridge.OverflowException();}
return i;};Decimal.min=function(a,b){return a.compareTo(b)<0?a:b;};Decimal.max=function(a,b){return a.compareTo(b)>0?a:b;};Decimal.tryParse=function(s,provider,v){try{v.v=new Bridge.Decimal(s,provider);return true;}catch(e){v.v=new Bridge.Decimal(0);return false;}};Decimal.toFloat=function(v){if(!v){return null;}
var otherDecimal=(v instanceof Decimal?v:new Decimal(v));return otherDecimal.toFloat();};if(typeof module!='undefined'&&module.exports){module.exports=Decimal;}
Bridge.Decimal=Decimal;Bridge.Decimal.Zero=Bridge.Decimal(0);Bridge.Decimal.One=Bridge.Decimal(1);Bridge.Decimal.MinusOne=Bridge.Decimal(-1);})();
