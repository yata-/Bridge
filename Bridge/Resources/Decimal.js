...
(function () {
    var decimal = {
        ...
        floor: function (a) {
            a = Bridge.Decimal.$(a);
            return Bridge.Decimal.$.round(a, 0, a.is_minus ? -2 : -1);
        },

        round: function (a, digits, mode) {
            if (!Bridge.hasValue(digits)) {
                digits = 0;
            }

            if (!Bridge.hasValue(mode)) {
                mode = 0;
            }
            a = Bridge.Decimal.$(a);
            return Bridge.Decimal.$.round(a, digits, mode);
        },

        trunc: function (a) {
            return Bridge.Decimal.$.round(Bridge.Decimal.$(a), 0, -1);
        },
...
// JSDecimal by Hiroshi Manabe (manabe.hiroshi #at# gmail #dot# com)
// The MIT License
// Copyright (c) 2013 Hiroshi Manabe
...
(function() {
    ...
    Decimal.prototype.addSub = function(another, is_sub) {
        var another_decimal = (another instanceof Decimal ? another : new Decimal(another));
        var abs_smaller;
        var abs_larger;
        var ret_is_minus;
    ...
    Decimal.prototype.mod = function (another) {
        var a = this.abs(),
            b = (another instanceof Decimal ? another : new Decimal(another)).abs(),
            r = a.sub(a.div(b).floor().mul(b));
