    ...
    statics: {
        test: function () {
            ...
            input.onchange = Bridge.fn.combine(input.onchange, function (ev) {
                ...
                Bridge.get(console).log("ev.CurrentTarget.Value: " + ev.currentTarget.value);

                // Tests if ev.IsMouseEvent() compiles
                Bridge.get(console).log("IsMouseEvent: " + Bridge.is(ev, MouseEvent));
            });
            ...
            anchor.onclick = Bridge.fn.combine(anchor.onclick, function (ev) {
                ...
                Bridge.get(console).log("ev.CurrentTarget.Href: " + ev.currentTarget.href);
            });

            // Test if Document.GetElementById<>() compiles
            var div = Bridge.get(document).getElementById("div1");
